...
def learn_d_z_multi(
    ...
    if algorithm == "stochastic":
        ...
        # for D.
        assert (
            "max_iter" not in solver_d_kwargs
            or solver_d_kwargs["max_iter"] != 1  # noqa
    ...
    with get_z_encoder_for(
        X, d_solver.D_hat, n_atoms, n_times_atom, n_jobs, solver_z, z_kwargs, reg
    ) as z_encoder:
        if callable(callback):
            callback(z_encoder, [])

        end_iter_func = get_iteration_func(
            eps, stopping_pobj, callback, lmbd_max, name, verbose, raise_on_increase
        )
        ...
        if algorithm == "batch":
            pobj, times = _batch_learn(greedy=False, folder = folder, **kwargs)
        elif algorithm == "greedy":
            ...
            raise NotImplementedError(
                "Algorithm '{}' is not implemented to learn dictionary atoms.".format(
                    algorithm
                )
            )
...
def _batch_learn(
    ...
    if greedy:
        ...
        if n_iter < n_atoms * n_iter_by_atom:
            raise ValueError(
                "The greedy method needs at least %d iterations "
                "to learn %d atoms. Got only n_iter=%d. Please "
                "increase n_iter." % (n_iter_by_atom * n_atoms, n_atoms, n_iter)
            )
    ...
    for ii in range(n_iter):  # outer loop of coordinate descent
        ...
        if greedy and ii % n_iter_by_atom == 0 and d_solver.D_hat.shape[0] < n_atoms:
            # add a new atom every n_iter_by_atom iterations
            d_solver.add_one_atom(z_encoder)

        if lmbd_max in ["per_atom", "shared"] or (lmbd_max == "scaled" and ii == 0):
            z_encoder.update_reg(lmbd_max == "per_atom")

            if verbose > 5:
                print("[{}] lambda = {:.3e}".format(name, np.mean(z_encoder.reg)))
        ...
        if np.all(z_nnz == 0):
            ...
            warnings.warn(
                "Regularization parameter `reg` is too large "
        ...
        if (not greedy or d_solver.D_hat.shape[0] == n_atoms) and end_iter_func(
            z_encoder, pobj, ii
        ):
            break
    ...
    def end_iteration(z_encoder, pobj, iteration):
        ...
        if (dz < eps or du < eps) and lmbd_max in ["fixed", "scaled"]:
            if dz < 0 and raise_on_increase:
                raise RuntimeError(
                    "The z update have increased the objective value by {}.".format(dz)
                )
            ...
            if dz < eps and du < eps:
                if verbose == 1:
                    print("")
                print(
                    "[{}] Converged after {} iteration, (dz, du) "
